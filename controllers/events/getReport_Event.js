...
async function getReport_EventStatus(req, res) {

    var token = localStorage.getItem('token');
    const url = 'https://186.29.71.205/ciegis/backend/index.php/api/dsktp/events/all_events/' + token;
    ...
    xhr.onload = function() {
        if (xhr.readyState === xhr.DONE) {
            if (xhr.status === 200) {
                ...
                console.log(data.eventData.lenght);
                res.render('events/reportEventStatus', { eventData: data.eventData });
            }
        }
    };
...
module.exports = getReport_EventStatus;