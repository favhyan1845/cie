...
function getAsociatedIncident(req, res) {
    var objID = JSON.parse(JSON.stringify(req.body)); //Parse ID from request body
    var id = objID["id"]; //obtain the id value
    var token = localStorage.getItem('token');
    const url = 'https://186.29.71.205/ciegis/backend/index.php/api/dsktp/incidents/associated/' + id + '/' + token;
    ...
    xhr.onload = function() {
        ...
        if (jsonResponse.code == 200) {
            //console.log(id);
            res.render('incidents/asociatedIncident', {
                eventinfo: jsonResponse.incidentData
...
module.exports = getAsociatedIncident;